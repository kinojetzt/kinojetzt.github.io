...
function entryCardHTML(e, num){
  ...
  return `
    <article class="entry glass card" data-id="${e.id}">
      <div class="entry-number">${num}</div>
      ${poster}
      <div>
...
// ------- CRUD -------
function addEntry(entry){ const entries = load(); entries.push(entry); save(entries); render(); }
function updateEntry(id, patch){
  const entries = load();
  const idx = entries.findIndex(e=>e.id===id);
  if (idx>=0){ entries[idx] = {...entries[idx], ...patch}; save(entries); render(); }
}
function removeEntry(id){ const entries = load().filter(e => e.id !== id); save(entries); render(); }
